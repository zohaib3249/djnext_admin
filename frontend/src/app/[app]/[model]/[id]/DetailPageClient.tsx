...
export default function DetailPageClient({
  ...
  useEffect(() => {
    ...
    const edit = new URL(window.location.href).searchParams.get('edit') === '1';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-foreground">
                {schema.model?.verbose_name ?? titleName(resolved.model)}
              </h1>
              <p className="text-sm text-muted-foreground">
                {(data as Record<string, string>)?._display ?? `#${resolved.id}`}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {schema.inlines.map((inline) => (
              <InlineTable
                ...
                parentAppLabel={resolved.app}
                parentModelName={resolved.model}
              />
            ))}
          </div>
        ) : null}
